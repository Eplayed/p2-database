...
本项目负责把 poe.ninja、poe2db、DD373、poe2ggg 和人工维护内容转成小程序可读取的 JSON，并上传 OSS。POE2 是当前主线；POE1 赛季助手使用独立目录和 OSS 前缀，绝不覆盖 POE2 产物。前端可以复用同一套抄 BD、看行情、解卡点页面，但数据产物必须按游戏线隔离。
...
Dashboard 页面是工作台布局：左侧固定菜单切换总览、自动运行、任务流程、内容研究、运行日志和功能调研；右侧展示对应操作区域。

### 本地自动运行队列

Dashboard 的“本地自动运行”不是后台服务，只在页面保持打开时生效。使用方式：

1. 选择任务并点击“添加到队列”。
2. 在“执行顺序”中拖拽排序，形成要定时跑的本地队列。
3. 设置间隔和浮动时间后开启自动运行；添加、移除、拖拽排序、开关和时间变更都会自动保存。
4. 到点会弹出 5 秒倒计时；确认后按顺序逐个运行，前一个任务完成后才启动下一个任务。

队列失败策略：任一任务失败、停止或启动失败，自动运行会关闭并中断后续任务。这样可以避免错误产物继续上传 OSS。运行中不可改队列，需等待任务结束后再调整。

天赋树截图属于可缺失资产：poe.ninja 页面偶尔不会渲染天赋树区域，或任务运行中图片文件被新一轮抓取替换。上传脚本遇到本地 `*_tree.jpg` 缺失时会记录告警并跳过该图片，不应弹出浏览器阻塞框；如果 JSON 数据已成功生成，队列可以继续完成后续上传。若大量玩家都缺天赋图，再回头排查抓取脚本。

队列设置同时保存在浏览器本地和 `dashboard/runtime/automation-settings.json`。刷新页面后会恢复最新设置；如果服务端还没有设置文件，会优先保留浏览器本地已有队列。
...
- POE1 与 POE2 的产物不能混写：POE2 使用 `translated-data/{env}` 与 `poe2-ladders/{env}/`；POE1 使用 `translated-data/poe1/{env}` 与 `poe1-season/{env}/`。
