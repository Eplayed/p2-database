...
- 找到国内玩家正在问什么。
- 观察海外攻略站和版本趋势。
- 把小程序相关选题归到三条主线：抄 BD、看行情、解卡点。
- 把自媒体相关选题归到新闻资讯和热点信息，方便公众号、头条号、论坛发帖做选题池。
- 为公众号、头条号和论坛帖提供候选主题。
...
```text
现有 QClaw 论坛采集 -> POE/暗黑/魔兽资讯与热点参考 -> 统一内容研究摘要
```

现有 QClaw 采集继续负责 D2Core 和踩蘑菇 POE2 论坛数据；p2-database 聚合脚本负责把论坛摘要、Maxroll、Wowhead、Blizzard 等参考源统一成 Dashboard 可读的选题池。
...
- `https://maxroll.gg/d4`
- `https://maxroll.gg/d4/news`
- `https://www.wowhead.com/news`
- `https://worldofwarcraft.blizzard.com/en-us/news`

这些来源只作为趋势、新闻和选题参考。脚本只保存标题、链接、标签和短摘要，不抓全文，不直接当事实来源。涉及补丁、活动、掉落、职业改动、时间节点时，写作前必须回到官方公告或游戏内文本核验。
...
- `game`：`poe1` / `poe2` / `d4` / `wow`
- `source`：来源名称
- `sourceType`：`forum` / `overseas_reference` / `news_reference` / `official_news`
- `title` / `titleCn`
- `url`
- `tags`
- `signals.painPoint`
...
- 生成 `scoreBreakdown`，能看到分数来自开荒、抄 BD、看行情、解卡点、新闻资讯、热点信息、赛季、论坛、海外参考或官方资讯。
- 生成 `signals.miniapp`，标明这个选题应该承接到“抄 BD / 看行情 / 解卡点”哪条小程序主线，或作为“新闻资讯 / 热点信息”进入自媒体选题池。
- 导出 `dashboard/runtime/content-research-topics.md`，可以直接拿去做公众号、头条号或论坛选题筛选。

Dashboard 会额外展示一块“内容研究看板”：

- `今天优先写`：按分数列出最适合先写的选题，并标明来源、游戏、内容方向和核验动作。
- `内容方向`：按 `抄BD`、`看行情`、`解卡点`、`新闻资讯`、`热点信息`、`内容观察` 聚合数量和高分标题。
- `筛选结果`：支持按内容方向和游戏筛选，方便每天只看 POE1、POE2、暗黑破坏神或魔兽世界相关选题。

使用方式：

1. 先运行 Dashboard 的 `内容研究 -> 更新论坛选题池`。
2. 看 `今天优先写` 里是否有能承接小程序入口的题。
3. 小程序功能落点仍优先回到三条主线之一；如果是 `新闻资讯` 或 `热点信息`，优先作为文章选题处理，不急着改小程序。
...
- 连续出现的 `新闻资讯 / 热点信息`，优先做公众号、头条号、论坛短帖或热点跟踪。
