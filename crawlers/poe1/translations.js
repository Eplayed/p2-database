...
function translateStatText(value) {
  ...
  const customPatterns = [
    ...
    [/^\+([\d.]+)% to maximum (Fire|Cold|Lightning|Chaos) Resistance$/, (_, number, type) => `${replaceKeywords(type)}抗性上限 +${number}%`],
    [/^\+([\d.]+)% to maximum (火焰|冰霜|闪电|混沌)抗性$/, (_, number, type) => `${type}抗性上限 +${number}%`],
    ...
    [/^Cannot be Stunned$/, () => '不会被眩晕'],
    [/^-([\d.]+)% to maximum Chance to Block Attack Damage$/, (_, number) => `攻击伤害格挡几率上限 -${number}%`],
    [/^-([\d.]+)% to maximum Chance to Block Spell Damage$/, (_, number) => `法术伤害格挡几率上限 -${number}%`],
    [/^\+([\d.]+)% Chance to Block Spell Damage for each 1% Overcapped Chance to Block Attack Damage$/, (_, number) => `每 1% 溢出的攻击伤害格挡几率，获得 +${number}% 法术伤害格挡几率`],
    [/^Converts all Evasion Rating to Armour\. Dexterity provides no bonus to Evasion Rating$/, () => '将所有闪避值转化为护甲。敏捷不再提供闪避值加成'],
    [/^All Damage taken bypasses Energy Shield$/, () => '承受的所有伤害绕过能量护盾'],
    [/^([\d.]+)% of Elemental Damage taken as Chaos Damage$/, (_, number) => `${number}% 承受的元素伤害转为混沌伤害`],
